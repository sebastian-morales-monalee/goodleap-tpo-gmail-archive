...
- Shows the count inside each segment of all three stacked charts, with
  descriptive legends. Email Production labels use a persistent high-contrast
  cyan color; project status labels remain black against the light status
  colors.
...
- Enriches columns D:J from PostHog with Address, State, Region, Solar Panel,
  ...
  the assigned team.
- Creates and maintains `US State Regions` as a local State-to-Region reference
  table. Alaska, Hawaii, and Puerto Rico remain separate from Northeast,
...
4. Open `ProjectIdSummary.gs` and run
   ...
   source, address, state, equipment, installer, team, map data source, RGB URL,
...
6. Run `setupProjectIdSummary()` once. It safely inserts Application ID after
   ...
   `US State Regions` and `Status Order` references; and preserves all existing values while moving
   ...
   Sales fallback, `Status Order` matches its numeric lookup in the new
...
7. Run `refreshProjectIdSummary()` a second time. The expected result includes
   ...
   numeric values. The stage sequence is orientative, not a guaranteed linear
   lifecycle.
8. Run `refreshAIAnalysisDashboard()` once to rebuild all three weekly stacked
   charts, including the project-production charts sourced from `Created At`
   and `Updated At` with the tolerance status.
...
- `AI Weekly Summary` contains all Monday-to-Sunday category aggregates in
  ...
  Projects` reconciles every unique project with a valid `Created At`, while
